/*
 * jQuery DP Social Timeline v1.6.6
 *
	...
	SocialTimeline.prototype = {
		...
		_updateTimelineSize : function() {
			...
			if(this.timeline.width() < 480) {
				...
				$('.dpSocialTimeline_item', this.timeline).css('width', '98%');
		...
		_parseRSS : function(url, limit, int_val, fnk, num){
			...
			if(url.indexOf('user_timeline.php') !== -1 || url.indexOf('search.php') !== -1 || url.indexOf('instagram.php') !== -1) {
				gurl = url;	
		...
		_relativeTime : function(time_value) {
			...
			var r = '';
			if (delta < 60) { //60 sec
				r = 'Il y a mois d\’une minute';
			} else if(delta < 120) { //2 min
				r = 'Il y a environ une minute';
			} else if(delta < (60*60)) { //60 min
				r = 'Il y a environ ' + (parseInt(delta / 60)).toString() + ' minutes';
			} else if(delta < (120*60)) { //2 hours
				r = 'Il y a environ 1 heure';
			} else if(delta < (24*60*60)) { //1 day
				r = 'Il y a environ ' + (parseInt(delta / 3600)).toString() + ' heures';
		...
		_getImage : function(entry, returnSrc){
			...
			if(backRefs.length > 0 && backRefs[0].indexOf('sndcdn.com') === -1) {
				...
				image = image.replace("/v/t1.0-9/", "/t1.0-9/");
